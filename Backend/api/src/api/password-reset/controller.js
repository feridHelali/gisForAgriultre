...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your gisForAgriculture account.<br>
        ...
        &mdash; gisForAgriculture Team
      `
      return sendMail({ toEmail: email, subject: 'gisForAgriculture - Password Reset', content })
    })
